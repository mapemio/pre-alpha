...
function generaMiniMap()
{
	...
	carga=$.ajax({
		...
		{ 
			...
			alert("Miniatura generada correctamente!");
...
function generaMapa(mapName, w, h, parcela_descripcion,parcela_borde,background_resize, plantilla)
{
  ...
  */
  //alert("Nombre del mapa a generarse="+ mapName + " y parcela_descripcion=" + parcela_descripcion);
	datos="nombre=" + mapName + "&num=" + w + "&num2=" + h + "&doGenerate=Generate" + "&parcela_descripcion=" + parcela_descripcion + "&parcela_borde=" + parcela_borde + "&background_resize=" + background_resize + "&plantilla=" + plantilla;
...
function botonMenu()
{
	...
	if($(document).find('#panel').css('left')=='0px') 
	{
		...
		document.getElementById('leftRightIcon').src='icons/1361402296_RightArrow.png'
	...
	{
		$("#panel").animate({"left": "+=400px"}, "fast");
		document.getElementById('leftRightIcon').src='icons/1361402303_LeftArrow.png'
